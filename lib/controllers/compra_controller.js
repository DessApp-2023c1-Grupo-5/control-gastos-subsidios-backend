...
export const getCompraTotalAprobado = async (req, res) => {
  const id = Number(req.params.id);
  const compras = await Compras.findAll({
    include: [{ model: Proyecto }],
  });
  console.log(compras.map((compra) => compra.toJSON()));
  const comprasById = compras.filter((compra) => compra.idProyecto === id);
...
export const getCompraTotalPendienteOtro = async (req, res) => {
  const id = Number(req.params.id);
  const compras = await Compras.findAll({
    include: [{ model: Proyecto }],
  });
...
export const getCompraTotalPendiente = async (req, res) => {
  const id = Number(req.params.id);
